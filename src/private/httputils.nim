...
proc respGen*(statusLine, mime, body: string): string {.inline.} =
  result =
    fmt("{statusLine}" &
...
proc respGen*(statusLine, mime, body, charset: string): string {.inline.} =
  result =
    fmt("{statusLine}" &
        "Server: {serverName}\r\l" &
        "Date: {serverTime}\r\l" &
...
proc respGenWithHeader*(statusLine, mime, body: string,
...
proc respGenWithHeader*(statusLine, mime, body, charset: string,
                        headers: openArray[tuple[name: string, value: string]]): string {.inline.} =
  result =
    fmt("{statusLine}" &
        "Server: {serverName}\r\l" &
...
proc redirectTo*(URL: string): string {.inline.} =
  result = respGenWithHeader(HTTP301, "text/html", "", [("Location", URL)])

proc badRequest*(): string {.inline.} =
  result = respGen(HTTP400, "text/html", "")

proc notFound*(): string {.inline.} =
  result = respGen(HTTP404, "text/html", "404 Not Found")

proc bodyTooLarge*(): string {.inline.}=
  result = respGen(HTTP413, "text/html", "413 Request Entity Too Large")